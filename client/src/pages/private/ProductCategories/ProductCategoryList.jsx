...
const ProductCategoryList = () => {
  ...
  const fetchCategories = async () => {
    try {
      const { data } = await getApiProductCategories();
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await Promise.all(selectedCategories.map(id => deleteProductCategory(id)));
      alert("✅ Các danh mục đã được xóa thành công!");
      setSelectedCategories([]);
      fetchCategories();
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <Package className="text-main" /> Danh Mục Sản Phẩm
        </h2>
                ...
                <tr key={category._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex gap-2">
                      <Link
                        to={`/admin/product-categories/edit/${category._id}`}
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(endIndex, filteredCategories.length)}</span> trong{" "}
                  <span className="font-medium">{filteredCategories.length}</span> kết quả
                </p>
              </div>
              <div>
      ...
      {filteredCategories.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {search ? "Thử điều chỉnh từ khóa tìm kiếm." : "Bắt đầu bằng cách tạo danh mục mới."}
          </p>
          {!search && (
            <div className="mt-6">
              <Link
