  ...
  const removeFilter = (keyToRemove, valueToRemove = null) => {
    setFilters((prevFilters) => {
      ...
      if (keyToRemove === 'color' || keyToRemove === 'size') {
        const currentValues = newFilters[keyToRemove] ? newFilters[keyToRemove].split(',') : [];
        ...
        if ((!newFilters['price[gte]'] || newFilters['price[gte]'] === 100000) && (!newFilters['price[lte]'] || newFilters['price[lte]'] === 50000000)) {
            delete newFilters['price[gte]'];
            delete newFilters['price[lte]'];
        }
  ...
  const renderFilterTags = () => {
    ...
    if (filters.color) {
      filters.color.split(',').forEach(c => {
        tags.push(
          <span key={`color-${c}`} className="flex items-center bg-blue-100 text-blue-800 text-sm font-medium px-2.5 py-0.5 rounded-full mr-2">
            Color: {c}
            <button
    ...
    if (filters.size) {
      filters.size.split(',').forEach(s => {
        tags.push(
          <span key={`size-${s}`} className="flex items-center bg-green-100 text-green-800 text-sm font-medium px-2.5 py-0.5 rounded-full mr-2">
            Size: {s}
            <button
    ...
    // Price range filters
    const minPrice = filters['price[gte]'] ? Number(filters['price[gte]']) : null;
    const maxPrice = filters['price[lte]'] ? Number(filters['price[lte]']) : null;
    const defaultMinPrice = 100000;
    const defaultMaxPrice = 50000000;

    if (minPrice !== null && (minPrice !== defaultMinPrice || maxPrice !== defaultMaxPrice)) {
      // Chỉ hiển thị tag giá nếu có sự thay đổi so với mặc định
      const priceTagContent = (minPrice && maxPrice) 
        ? `Price: ${minPrice.toLocaleString('vi-VN')} - ${maxPrice.toLocaleString('vi-VN')} VND`
        : (minPrice ? `From: ${minPrice.toLocaleString('vi-VN')} VND` : `To: ${maxPrice.toLocaleString('vi-VN')} VND`);

      tags.push(
        <span key="price-range" className="flex items-center bg-purple-100 text-purple-800 text-sm font-medium px-2.5 py-0.5 rounded-full mr-2">
          {priceTagContent}
          <button
            onClick={() => {
    ...
    if (filters.sort) {
      tags.push(
        <span key="sort" className="flex items-center bg-yellow-100 text-yellow-800 text-sm font-medium px-2.5 py-0.5 rounded-full mr-2">
